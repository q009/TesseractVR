...
extern void movevrplayer(physent *pl);
...
extern void vecfromyawpitch(float yaw, float pitch, const vec &movedir, vec &m);
extern void vecfromyawpitch(float yaw, float pitch, float move, float strafe, vec &m);
...
extern void rendermodel(const char *mdl, int anim, const vec &o, float yaw = 0, float pitch = 0, float roll = 0, int cull = MDL_CULL_VFC | MDL_CULL_DIST | MDL_CULL_OCCLUDED, dynent *d = NULL, modelattach *a = NULL, int basetime = 0, int basetime2 = 0, float size = 1, const vec4 &color = vec4(1, 1, 1, 1), const quat &baseorient = quat(0, 0, 0, 1));
extern int intersectmodel(const char *mdl, int anim, const vec &pos, float yaw, float pitch, float roll, const vec &o, const vec &ray, float &dist, int mode = 0, dynent *d = NULL, modelattach *a = NULL, int basetime = 0, int basetime2 = 0, float size = 1, const quat &baseorient = quat(0, 0, 0, 1));
